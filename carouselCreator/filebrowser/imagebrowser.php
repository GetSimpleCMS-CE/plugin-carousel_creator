	...
	<div class="wrapper">
		<div id="maincontent">
			<div class="main" style="border:none;">
					...
					<tbody>
						...
						<tr class="All">
							<td class="" colspan="5">
								...
								<a href="imagebrowser.php?path=<?php echo $p; ?>&amp;func=<?php echo $func; ?>&amp;w=<?php echo $w; ?>&amp;h=<?php echo $h; ?>&autoclose=1&CKEditor=post-content&class=<?php echo $_GET['class'];?>&func=<?php echo $_GET['func'];?>" title="<?php echo $upload['name']; ?>"><strong><?php echo $upload['name']; ?></strong></a>
							</td>
						</tr>
						<?php
								}
						...
						<tr class="All images">
							...
							<td style="white-space:nowrap;">
								<span><?php echo $upload['width']; ?> x <?php echo $upload['height']; ?></span>
							</td>
							...
							<td style="width:70px;text-align:right;">
								<span><?php echo $fileOwner['name']; ?>/<?php echo $filePerms; ?></span>
							</td>
							<?php } ?>
							<td style="width:85px;text-align:right;">
								<span><?php echo shtDate($upload['date']); ?></span>
							</td>
						</tr>
						<?php if ($debug) echo '
				...
				<script version="35343356324">
				function submitLink(e) {
					let linker = document.querySelectorAll('.images img')[e].getAttribute('src');
					console.log(linker);
					let linkerNew = linker;

					if(window.opener.document.querySelectorAll('.thumbnails')[<?php echo $_GET['func'];?>]){
						window.opener.document.querySelectorAll('.thumbnails')[<?php echo $_GET['func'];?>].setAttribute('src',linker);
					};

					window.opener.document.querySelectorAll('.<?php echo $_GET['class'];?>')[<?php echo $_GET['func'];?>].value = linker;
